...
import { frag } from '../lib/tags';

const { width, height } = Dimensions.get('window');
const origin = { x: width / 2, y: height / 2 };
const c1 = Skia.Color('#3f0a0b');
const c2 = Skia.Color('#f43f5e');
const bpm = 44;
...
vec4 main(vec2 xy) {
  ...
  p *= 3; // Ajuste a escala do coração aqui, valores menores que 1.0 diminuem o coração
  p.y += -2;
  float d = sdHeart(p);
  if (d < 0) {
    vec3 col = c1.rgb;
...
export const Heartrate = () => {
  ...
  return (
    <Canvas style={{ height: 320, width: 400 }}>
      <Fill color="black" />
      {values.map((val, i) => (
